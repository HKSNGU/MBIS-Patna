import "bootstrap/dist/css/bootstrap.css";
import "bootstrap/dist/js/bootstrap.bundle";
import styles from "./Nav1.module.css";

function Nav1() {
  return (
    <>
      <nav
        className={`navbar navbar-expand-lg bg-body-tertiary ${styles["nav1"]}`}
      >
        <div className={`container-fluid ${styles["nav1_container"]}`}>
          <ul className={`navbar-nav me-auto ${styles["left"]}`}>
            <li class="nav-item">
              <a className={`nav-link ${styles["link"]}`} href="#">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="15.5"
                  width="15.5"
                  viewBox="0 0 512 512"
                  className={`${styles["icon"]}`}
                >
                  <path
                    fill="#96a2b8"
                    d="M64 208.1L256 65.9 448 208.1v47.4L289.5 373c-9.7 7.2-21.4 11-33.5 11s-23.8-3.9-33.5-11L64 255.5V208.1zM256 0c-12.1 0-23.8 3.9-33.5 11L25.9 156.7C9.6 168.8 0 187.8 0 208.1V448c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V208.1c0-20.3-9.6-39.4-25.9-51.4L289.5 11C279.8 3.9 268.1 0 256 0z"
                  />
                </svg>
                info@mbispatna.org
              </a>
            </li>
            <li class="nav-item">
              <a className={`nav-link ${styles["link"]}`} href="#">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  height="15.5"
                  width="15.5"
                  viewBox="0 0 512 512"
                  className={`${styles["icon"]}`}
                >
                  <path
                    fill="#96a2b8"
                    d="M164.9 24.6c-7.7-18.6-28-28.5-47.4-23.2l-88 24C12.1 30.2 0 46 0 64C0 311.4 200.6 512 448 512c18 0 33.8-12.1 38.6-29.5l24-88c5.3-19.4-4.6-39.7-23.2-47.4l-96-40c-16.3-6.8-35.2-2.1-46.3 11.6L304.7 368C234.3 334.7 177.3 277.7 144 207.3L193.3 167c13.7-11.2 18.4-30 11.6-46.3l-40-96z"
                  />
                </svg>
                +91-8826273838, +91-8102450507
              </a>
            </li>
          </ul>
          <ul className={`navbar-nav ms-auto ${styles["right"]}`}>
            <li class="nav-item">
              <a className={`nav-link ${styles["link"]}`} href="#">
                Registration
              </a>
            </li>
            <li class="nav-item">
              <a className={`nav-link ${styles["link"]}`} href="#">
                Admission Enquiry
              </a>
            </li>
          </ul>
        </div>
      </nav>
    </>
  );
}

export default Nav1;
